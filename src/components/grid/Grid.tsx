...
export const Grid = ({ cells, rows, cols, title }: Props) => {
  const renderGrid = cells.map((cell) => {
    const key = cell.key;
    return <StyledCell key={key}>{cell.state}</StyledCell>;
  });
  ...
  return (
    <StyledWrapper>
      ...
      <StyledLeftMarginWrapper>
        <StyledRowWrapper>{renderRowIndexs}</StyledRowWrapper>
      </StyledLeftMarginWrapper>
      <StyledRowWrapper>
        <StyledColWrapper>{renderColIndexs}</StyledColWrapper>
        <StyledGrid cols={cols}>{renderGrid}</StyledGrid>
      </StyledRowWrapper>
    </StyledWrapper>
  );
};
